...
class Player:
    ...
    def next_move(self, board):
        """Returns the index of the best column for the next move for (self) player in (board) board"""
        return self.tiebreak_move(self.scores_for(board))
...
#
# Tests Player.next_move(board)
#
assert Player('X', 'LEFT', 1).next_move(board1) == 0
assert Player('X', 'RIGHT', 1).next_move(board1) == 6
assert Player('X', 'LEFT', 2).next_move(board1) == 3
assert Player('X', 'RIGHT', 2).next_move(board1) == 3
assert Player('X', 'RANDOM', 2).next_move(board1) == 3
